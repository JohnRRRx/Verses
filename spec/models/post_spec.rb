...
RSpec.describe Post, type: :model do
  ...
  describe '機能チェック' do
    it 'タグの追加、保存可能' do
      post = build(:post, tag_list: %w[tag1 tag2 tag3])
      post.save
    ...
    it '同じタグを重複追加不可' do
      post = build(:post, tag_list: %w[tag1 tag1])
    ...
    it 'タグを使って投稿を検索可能' do
      ...
      post3 = create(:post, tag_list: %w[tag1 tag2])
    ...
    it 'タグ削除可能' do
      post = build(:post, tag_list: %w[tag1 tag2])
      post.save
