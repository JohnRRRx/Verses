class SpotifySearchHandler {
  ...
  initialize() {
    ...
    // 検索結果コンテナのスタイリング
    this.searchResults.className = 'mt-3 border rounded-lg bg-white shadow-lg overflow-hidden mb-20'; // shadowを強調
      ...
      div.innerHTML = `
        <button type="button"
          ...
          <div class="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-md overflow-hidden">
            <img src="${track.image_url || '/api/placeholder/48/48'}" 
    ...
    this.searchResults.innerHTML = `
      <div class="p-4">
        <div class="bg-green-50 text-green-700 p-4 rounded-lg flex items-center gap-3">
          <div class="flex-shrink-0 w-12 h-12 bg-green-100 rounded-md overflow-hidden">
            <img src="${track.image_url || '/api/placeholder/48/48'}" 
                 alt="${track.album || 'Album'} cover" 
                 class="w-full h-full object-cover"
                 onerror="this.src='/api/placeholder/48/48'">
