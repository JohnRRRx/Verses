class SpotifySearchHandler {
  ...
  setupResizeHandler() {
    // 画面サイズに応じて検索結果の最大高さを調整
    const updateMaxHeight = () => {
      const vh = window.innerHeight;
      const searchResultsTop = this.searchResults.getBoundingClientRect().top;
      const maxHeight = vh - searchResultsTop - 20; // 下部に20pxの余白
      this.searchResults.style.maxHeight = `${maxHeight}px`;
    };

    window.addEventListener('resize', updateMaxHeight);
    updateMaxHeight(); // 初期設定
  }
  ...
  addStyles() {
    ...
    style.textContent = `
      .search-results-scroll {
        ...
        max-height: calc(100vh - 16rem); /* デフォルトの最大高さ */
      }
      @media (max-width: 640px) {
        .search-results-scroll {
          max-height: calc(100vh - 12rem); /* モバイルでの最大高さ */
        }
      }
