class SpotifySearchHandler {
  ...
  initialize() {
    ...
    // 検索結果コンテナのスタイリング
    this.searchResults.className = 'mt-3 border rounded-lg bg-white shadow-sm overflow-hidden mb-20'; // フッター用の余白を確保
  ...
  displayResults(data) {
    ...
    data.forEach(track => {
      ...
      div.innerHTML = `
        <button type="button"
                class="w-full text-left px-4 py-3 rounded-lg transition-colors duration-200 flex items-center gap-3 hover:bg-blue-50">
          <div class="flex-shrink-0 w-10 h-10 bg-gray-100 rounded-md flex items-center justify-center">
            🎵
          </div>
