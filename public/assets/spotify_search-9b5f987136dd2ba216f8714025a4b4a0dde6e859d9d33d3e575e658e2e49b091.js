class SpotifySearchHandler {
  ...
  initialize() {
    ...
    if (this.mainForm && this.submitButton) {
  ...
  handleSubmitClick(event) {
    const songId = document.getElementById('post_song_id').value;
    if (!songId) {
      event.preventDefault();
      this.showSongError();
    }
  ...
  showSongError() {
    if (this.songError) {
      this.songError.textContent = '曲を選択してください';
      this.songError.classList.remove('hidden');
  ...
  selectSong(track) {
    ...
    if (this.songError) {
      this.songError.classList.add('hidden');
    }
  }
}
